...
array = [16, 3, 12, 9, 1, 60, 1, 3, 14, 39, 13, 16, 16, 1, 9, 13, 3, 39, 60,
...
def random_dict(char, r=True):
    ...
    match n:
        ...
        case _:
            d = random_dict(chr(n - 1), False)
            m = rd.randint(0, 9)
            return d | {m: d[m] + rd.randint(0, 2)}


def shuffle_guess(guess, n):
    match list(guess):
        case []:
            return []
        case [f, *rest]:
            l = list(random_dict(f).values()) + shuffle_guess(''.join(rest), n*2)
            rd.seed(n)
            rd.shuffle(l)
            return l


def match_password(p, n=0):
    match p:
        case []:
            return n != 0
        case [f, *rest]:
            rd.seed(array[n])
            return rd.randint(0, 30) == f and match_password(rest, n + 1)


def test():
    """Test if the password is right."""
    if match_password(shuffle_guess(input("password:"), 1)):
...
def unshuffle(arr, s):
    ...
    rd.shuffle(perm)
    return [arr[perm.index(x)] for x in range(len(arr))]


def unshuffle_guess(shuffled, n=1):
    match shuffled:
        case []:
            return []
        case _:
            unshuffled = unshuffle(shuffled, n)
            return unshuffled[:10] + unshuffle_guess(unshuffled[10:], 2*n)


if __name__ == "__main__":
    len_flag = len(array)//10
    random_array = []
    for x in array:
        rd.seed(x)
        random_array.append(rd.randint(0, 30))

    available_key = []
    available_value = []
    for c in string.printable:
        available_key.append(c)
        available_value.append(list(random_dict(c).values()))

    unshuffled = unshuffle_guess(random_array)
    print(unshuffled)
    flag = ''
    for i in range(len_flag):
        sub = unshuffled[10*i:10*(1+i)]
        flag += available_key[available_value.index(sub)]
    print(flag)
